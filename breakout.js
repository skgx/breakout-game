...
const game = new Phaser.Game(config1);

function preload() {
    this.load.image('paddle', 'img/paddle.png');
    this.load.image('brick', 'img/TT.png');
    this.load.image('destroyed', 'img/curl1.png');
    this.load.image('ball', 'img/ball.png');
}
//setBounce(x [, y])
// Sets the bounce values of this body.
// Bounce is the amount of restitution, or elasticity, the body has when it collides with another object.
//  A value of 1 means that it will retain its full velocity after the rebound. A value of 0 means it will
//   not rebound at all.
...
function create() {
    ...
    bricks = this.physics.add.staticGroup({
        key: 'brick',
        frameQuantity: 20,
        gridAlign: { width: 10, cellWidth: 100, cellHeight: 100, x: this.cameras.main.centerX -277.5, y: 100 }
    });
